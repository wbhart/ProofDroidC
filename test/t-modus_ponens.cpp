...
struct TestCase {
    std::string implication;                  // Implication formula (e.g., "P(x) -> Q(x)")
    std::vector<std::string> unit_clauses;    // List of unit clause formulas (e.g., {"P(a)", "Q(b)"})
    std::string expected_result;              // Expected result formula after applying modus ponens
...
bool run_test_case(int test_number, context_t& ctx_var, const TestCase& test) {
    ...
    // Apply modus ponens
    node* result = modus_ponens(ctx_var, parsed_implication, parsed_units);
    ...
    if (!passed) {
        ...
        std::cout << "Implication: " << parsed_implication->to_string(REPR) << "\n";
        std::cout << "Unit Clauses: ";
        for (const auto& unit : parsed_units) {
            std::cout << unit->to_string(REPR) << " ";
        }
...
int main() {
    ...
    std::vector<TestCase> test_cases = {
        // Test Case 1: Single conjunct and single unit clause
        {
            "P(a) \\implies Q(a)",
            {"P(a)"},
            "Q(a)",
        ...
        // Test Case 2: Multiple conjuncts and multiple unit clauses
        {
            "P(x) \\wedge Q(y) \\implies R(x)",
            {"P(a)", "Q(b)"},
            "R(a)",
            false
        },
        // Test Case 3: Multiple conjuncts with variables and unit clauses
        {
            "P(x) \\wedge Q(y) \\implies R(z)",
            {"P(a)", "Q(b)"},
            "R(z)",
            false
        },
        // Test Case 4: Multiple conjuncts with functions and unit clauses
        {
            "P(f(x)) \\wedge Q(g(y)) \\implies R(h(z))",
            {"P(f(a))", "Q(g(b))"},
            "R(h(z))",
            false
        },
        // Test Case 5: Single conjunct and single unit clause with variable renaming
        {
            "P(x) \\implies Q(x)",
            {"P(y)"},
            "Q(y)",
        ...
        // Test Case 6: Nested conjunctions and unit clauses
        {
            "P(x) \\wedge Q(x) \\wedge R(x) \\implies S(x)",
            {"P(x)", "Q(x)", "R(x)"},
            "S(x)",
            false
        },
        // Test Case 7: Implication with nested functions and unit clauses
        {
            "P(f(x)) \\wedge Q(g(y)) \\implies R(h(z))",
            {"P(f(a))", "Q(g(b))"},
            "R(h(z))",
        ...
        // Test Case 8: Implication with multiple variables and unit clauses
        {
            "P(x, y) \\wedge Q(y, z) \\implies R(x, z)",
            {"P(a, b)", "Q(b, c)"},
            "R(a, c)",
            false
        }
